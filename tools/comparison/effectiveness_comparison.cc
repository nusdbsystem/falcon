...
int main(int argc, char *argv[]) {
  ...
  cout << "number of line of the first file: " << n_line_first << endl
       << "number of attributes of the first file: " << n_attributes_first
       << endl;
  cout << "number of line of the second file: " << n_line_second << endl
       << "number of attributes of the second file: " << n_attributes_second
       << endl;

  if (alg == "mse") {
    if ((n_line_first != n_line_second) ||
        (n_attributes_first != n_attributes_second)) {
...
float mean_squared_error(std::vector<float> a, std::vector<float> b) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    // ", squared_error = " << squared_error << endl;
  }
