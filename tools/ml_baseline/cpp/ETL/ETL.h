...
public:
  // constructor
  ETL(string dataset, string delimiter)
  ...
  Eigen::MatrixXd CSVtoEigen(vector<vector<string>> dataString, int rows,
                             int cols);

  // using z-score normalization
  // after obtaining the mean, std
  Eigen::MatrixXd NormalizeZscore(Eigen::MatrixXd dataMat,
                                  bool normalizeTarget);
  ...
  auto Std(Eigen::MatrixXd dataMat)
      -> decltype(((dataMat.array().square().colwise().sum()) /
                   (dataMat.rows() - 1))
                      .sqrt());

  // train test split like sklearn
  tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd, Eigen::MatrixXd>
  TrainTestSplit(Eigen::MatrixXd data, float split_ratio);
