...
func (wk *TrainWorker) RunLimeInstanceSampleTask() {
	/**
		FL Engine requires:
	        ("party-id", po::value<int>(&party_id), "current party id")
	        ("party-num", po::value<int>(&party_num), "total party num")
	...
	// 3. generate many files store etc
	modelInputFile := common.TaskDataPath + "/" + wk.DslObj.Tasks.LimeInsSample.InputConfigs.DataInput.Data
	modelFile := common.TaskModelPath + "/" + wk.DslObj.Tasks.LimeInsSample.OutputConfigs.TrainedModel
	logFile := common.TaskRuntimeLogs + "-" + wk.DslObj.Tasks.LimeInsSample.AlgorithmName
	KeyFile := common.TaskDataPath + "/" + wk.DslObj.Tasks.LimeInsSample.InputConfigs.DataInput.Key
	modelReportFile := common.TaskModelPath + "/" + wk.DslObj.Tasks.LimeInsSample.OutputConfigs.EvaluationReport
	...
	cmd = exec.Command(
		...
		"--existing-key", fmt.Sprintf("%d", wk.DslObj.ExistingKey),
		"--key-file", KeyFile,
		"--algorithm-name", wk.DslObj.Tasks.LimeInsSample.AlgorithmName,
		"--algorithm-params", wk.DslObj.Tasks.LimeInsSample.InputConfigs.SerializedAlgorithmConfig,
		"--model-save-file", modelFile,
		"--model-report-file", modelReportFile,
		"--is-inference", fmt.Sprintf("%d", 0),
		"--inference-endpoint", common.EmptyParams,
	...
	// 4. execute cmd
	logger.Log.Println("[TrainWorker]: task lime sampling start")
	// by default, worker will launch executor resource by sub process
	if common.IsDebug != common.DebugOn {
		wk.Tm.CreateResources(resourcemanager.InitSubProcessManager(), cmd)
	}
...
func (wk *TrainWorker) RunLimeWeightTask() {
	...
	if wk.DslObj.DistributedTask.Enable == 1 && wk.DistributedRole == common.DistributedParameterServer {
		...
		logger.Log.Println("[TrainWorker]: no ps needed in running LimeWeightTask task, "+
			"current is worker=,", wk.WorkerID, "skip executing ...")
		return
