...
{
  (kubectl create configmap listener-config-$PARTY_NUMBER --from-env-file=$COMBINE_PROPERTIES &> $LOG_FILE_PATH)
...
sed -i -e "s/LISTENER_PORT/$LISTENER_NODE_PORT/g" $LISTENER_YAML || exit 1
sed -i -e "s/LISTENER_TARGET_PORT/$LISTENER_NODE_PORT/g" $LISTENER_YAML || exit 1
