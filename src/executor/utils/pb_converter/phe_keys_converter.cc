...
void serialize_phe_keys(djcs_t_public_key* phe_pub_key,
  ...
  char* g_str_c = mpz_get_str(NULL, 10, phe_pub_key->g);
  char* delta_str_c = mpz_get_str(NULL, 10, phe_pub_key->delta);
  ...
  if (phe_pub_key->n != NULL) {
    for (int i = 0; i < phe_pub_key->s + 1; i++) {
      char* ni_str_c = mpz_get_str(NULL, 10, phe_pub_key->n[i]);
  ...
  char * si_str_c = mpz_get_str(NULL, 10, phe_auth_server->si);
...
void deserialize_phe_keys(djcs_t_public_key* phe_pub_key,
  ...
  mpz_init(g);
  mpz_init(delta);
  mpz_set_str(g, deserialized_phe_keys.public_key_g().c_str(), 10);
  mpz_set_str(delta, deserialized_phe_keys.public_key_delta().c_str(), 10);
  ...
  for (int i = 0; i < deserialized_phe_keys.public_key_n_size(); i++) {
    mpz_t ni;
    mpz_init(ni);
    mpz_set_str(ni, deserialized_phe_keys.public_key_n(i).c_str(), 10);
  ...
  mpz_t si;
  mpz_init(si);
  mpz_set_str(si, deserialized_phe_keys.auth_server_si().c_str(), 10);
