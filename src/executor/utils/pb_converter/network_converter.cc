...
void serialize_network_configs(std::vector<std::string> ips,
  ...
  for (int i = 0; i < ips.size(); i++) {
    ...
    com::nus::dbsytem::falcon::v0::PortArray *executors_port_array = network_config.add_executor_executor_port_arrays();
...
void deserialize_network_configs(std::vector<std::string>& ips,
  ...
  for (int i = 0; i < ips_size; i++) {
    ...
    const com::nus::dbsytem::falcon::v0::PortArray& port_array = network_config.executor_executor_port_arrays(i);
...
void serialize_ps_network_configs(
  ...
  for (int i = 0; i < worker_ips.size(); i++) {
    com::nus::dbsytem::falcon::v0::Worker *worker = ps_network_config.add_workers();
