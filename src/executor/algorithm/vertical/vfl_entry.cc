...
void train_linear_regression(Party* party, const std::string& params_str,
  ...
  LinearRegressionBuilder linear_reg_builder(params, weight_size,training_data,
...
void launch_linear_reg_parameter_server(Party* party,
  ...
  auto linear_reg_model_builder = new LinearRegressionBuilder (params, weight_size,
...
// and run .train() .eval() methods, then save model
void train_logistic_regression(Party* party,
  ...
  int weight_size = party->getter_feature_num();
  double training_accuracy = 0.0, testing_accuracy = 0.0;
  LogisticRegressionBuilder log_reg_model_builder(params, weight_size,training_data,testing_data,
                                                  training_labels,testing_labels,
                                                  training_accuracy, testing_accuracy);
...
void launch_log_reg_parameter_server(
  ...
  // init log_reg_model instance
  auto log_reg_model_builder = new LogisticRegressionBuilder (params, weight_size,
      training_data,testing_data,
      training_labels,testing_labels,
      training_accuracy, testing_accuracy);
...
void train_decision_tree(Party *party,
    const std::string& params_str,
    const std::string& model_save_file,
    const std::string& model_report_file,
    int is_distributed_train, Worker* worker) {
...
//  params.dp_budget = 0.1;
  ...
  // here should receive the train/test data/labels from ps
    ...
    double split_percentage = SPLIT_TRAIN_TEST_RATIO;
    party->split_train_test_data(split_percentage,full_training_data,full_testing_data,
                                 full_training_labels,full_testing_labels);
  ...
  DecisionTreeBuilder decision_tree_builder(params,training_data,testing_data,
                                            training_labels,testing_labels,
                                            training_accuracy, testing_accuracy);
...
void launch_dt_parameter_server(Party* party,
    const std::string& params_str,
    const std::string& ps_network_config_pb_str,
    const std::string& model_save_file,
    const std::string& model_report_file) {
...
//  params.dp_budget = 0.1;
  ...
  auto decision_tree_builder = new DecisionTreeBuilder (params,training_data,testing_data,
                                                        training_labels,testing_labels,
                                                        training_accuracy, testing_accuracy);
...
//  params.dt_param.dp_budget = 0.1;
  ...
  RandomForestBuilder random_forest_builder(params,training_data,testing_data,
                                            training_labels,testing_labels,
                                            training_accuracy, testing_accuracy);
...
void train_gbdt(Party party, const std::string& params_str,
  ...
  GbdtBuilder gbdt_builder(params,training_data,testing_data,
                           training_labels,testing_labels,
                           training_accuracy, testing_accuracy);
