...
void BinomialDeviance::update_terminal_regions(Party party,
  ...
  party.ciphers_to_secret_shares(residuals, residuals_shares, size,
  ...
  party.ciphers_to_secret_shares(ground_truth_labels, ground_truth_labels_shares, size,
                                 ACTIVE_PARTY_ID, PHE_FIXED_POINT_PRECISION);
  ...
  EncodedNumber* pre_predictions = new EncodedNumber[size];
  decision_tree_builder.tree.predict(party, decision_tree_builder.getter_training_data(),
                                     size, pre_predictions);
  std::vector<double> pre_predictions_shares;
  party.ciphers_to_secret_shares(pre_predictions, pre_predictions_shares, size,
                                 ACTIVE_PARTY_ID, PHE_FIXED_POINT_PRECISION);
