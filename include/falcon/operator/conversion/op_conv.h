...
/**
 * convert 1-dimension ciphertext vector to secret shares securely, each party
 * will have a share Algorithm 1: Conversion to secretly shared value in paper
...
void truncate_ciphers_precision(const Party &party, EncodedNumber *ciphers,
                                int size, int req_party_id, int dest_precision);
 ...
 * eg,. party 1 calculate {<a1_share1>, <a1_share1>, <a1_share1>} * {[b1], [b2],
 * [b3]} finally, all party send local res to req_party_id party to aggregate
 *
 * @param party: the participating party
...
/**
 * MatShareMul: This function compute the cipher and secret shares
 * multiplication each party holds a secret share matrix and the same ciphers
 * eg,. share(M*N) * cipher(N*K) = RES(M*K)
 *
 * @param party: initialized party object
 * @param shares: secret shares that held by parties
 * @param ciphers: the common ciphers for multiplication, all party must have
 * this,
...
/**
 * This function compute the element-wise multiplication between cipher and
 * secret shares each party holds a secret share matrix and the same ciphers
 *
 * @param party: initialized party object
 * @param shares: secret shares that held by parties
